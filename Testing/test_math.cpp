...
protected:
    ...
    void TestComputePrecisionCholesky_NonIdentityCovariances() const {
        ...
        std::vector<Eigen::MatrixXd> expected = {
            (Eigen::MatrixXd(2, 2) << 0.17177833476190066, -0.24362417677019974,
             0.0, 0.25287169133777315).finished(),

            (Eigen::MatrixXd(2, 2) << 0.09887213601054862, -0.057416313809908096,
             0.0, 0.14862858798164388).finished(),

            (Eigen::MatrixXd(2, 2) << 0.11668136690747105, -0.06396386718928732,
             0.0, 0.15380716255671226).finished()
        };
...
TEST(MathTest, EstimateWeightedLogProbabilities_RealData) {
    ...
    std::vector<std::vector<double> > data_raw = load_data_from_file(
        "../data/3_2_-0.26_1.csv", expected_clusters, k);
